...
export default async function ArticlePage({ params }) {
  ...
  const viewCount = Math.floor(Math.random() * 1000) + 100; // Simulated dynamic data
      ...
      <article style={{
        ...
        <header style={{ marginBottom: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
            <h1 style={{ margin: 0, color: '#333', lineHeight: '1.2' }}>
              {article.title}
            </h1>
            <span style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#666' }}>
            <span>By <strong>{article.author}</strong> • Published {article.publishedAt}</span>
